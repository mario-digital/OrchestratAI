...
- [ ] Update generate_mock_response to include agent_status (AC: 1, 2, 3)
  - [ ] Import AgentStatus enum at top of `orchestratai_api/src/services/mock_data.py`
  - [ ] In generate_mock_response, create agent_status dict before return
  - [ ] Set orchestrator status to IDLE (it already routed)
  - [ ] Set selected agent status to COMPLETE (finished processing)
  - [ ] Set other 2 agents status to IDLE
  - [ ] Add agent_status to ChatResponse return value
  - [ ] Example: `{"orchestrator": "IDLE", "billing": "COMPLETE", "technical": "IDLE", "policy": "IDLE"}`

- [ ] Add cache_status to ChatMetrics generation (AC: 8)
  - [ ] In generate_mock_response metrics section, add cache_status field
  - [ ] Randomly choose from ["hit", "miss", "none"] with weights [0.3, 0.3, 0.4]
  - [ ] Update ChatMetrics instantiation to include cache_status parameter

- [ ] Create generate_query_analysis_log helper function (AC: 4)
...
- [ ] Create generate_vector_search_log helper function (AC: 5, 7)
...
- [ ] Create generate_cache_operation_log helper function (AC: 6)
...
- [ ] Refactor generate_mock_retrieval_logs to use new log generators (AC: 4, 5, 6)
...
- [ ] Create realistic document content snippet generator (AC: 7)
...
- [ ] Update generate_mock_response to maintain realistic metrics (AC: 9)
...
- [ ] Write unit tests for new mock generators (AC: 10)
...
- [ ] Integration test: Full ChatResponse validation (AC: 10)
