...
- [ ] Task 1: Create StreamError class (AC: 10)
  - [ ] Create or extend `lib/errors.ts` file
  - [ ] Define StreamError class extending Error
  - [ ] Add `retryable: boolean` property
  - [ ] Add `code: string` property for error categorization
  - [ ] Export error codes: `NETWORK_ERROR`, `SERVER_ERROR`, `TIMEOUT`, `PARSE_ERROR`
  - [ ] Write unit tests for error class

- [ ] Task 2: Add retry logic to useStreaming hook (AC: 1, 6, 7)
  - [ ] Update `hooks/use-streaming.ts`
  - [ ] Add retry state: `const [retryCount, setRetryCount] = useState(0)`
  - [ ] Add retry delay calculation: `Math.min(1000 * (2 ** retryCount), 4000)` (1s, 2s, 4s, max 4s)
  - [ ] In onerror handler, check if error is retryable
  - [ ] If retryable and retryCount < 3, wait delay then retry connection
  - [ ] If not retryable or max retries exceeded, call onError callback
  - [ ] Reset retryCount on successful connection
  - [ ] Write tests for retry logic

- [ ] Task 3: Implement exponential backoff (AC: 6, 7)
  - [ ] Create retry helper function: `retryWithBackoff(fn, maxRetries, baseDelay)`
  - [ ] Calculate delay: `baseDelay * (2 ** attempt)` (exponential)
  - [ ] Use `setTimeout` with calculated delay between retries
  - [ ] Log retry attempts: `console.log(\`Retry attempt ${attempt + 1} of ${maxRetries}\`)`
  - [ ] Return final error if all retries exhausted
  - [ ] Write tests for backoff calculation

- [ ] Task 4: Add fallback mode state to ChatProvider (AC: 3, 12)
  - [ ] Update `components/providers/chat-provider.tsx`
  - [ ] Add `useFallbackMode: boolean` to state
  - [ ] Initialize to false (try streaming first)
  - [ ] Add reducer action: `SET_FALLBACK_MODE`
  - [ ] Persist fallback preference in sessionStorage
  - [ ] Check sessionStorage on mount to restore preference
  - [ ] Write tests for fallback mode state

- [ ] Task 5: Implement fallback in sendStreamingMessage (AC: 2, 4, 5)
  - [ ] In sendStreamingMessage method, check if useFallbackMode is true
  - [ ] If true, call sendMessage (non-streaming POST) instead
  - [ ] In onError callback from useStreaming:
  ...
  - [ ] Ensure fallback message appears correctly in chat
  - [ ] Write tests for fallback behavior

- [ ] Task 6: Add error categorization (AC: 10, 11)
  - [ ] In useStreaming onerror handler, categorize error:
  ...
  - [ ] Create appropriate StreamError with code and retryable flag
  - [ ] Network errors: retryable = true
  - [ ] Server errors (5xx): retryable = true
  - [ ] Client errors (4xx): retryable = false
  - [ ] Parse errors: retryable = false
  - [ ] Write tests for error categorization

- [ ] Task 7: Improve error toast messages (AC: 11)
  - [ ] Import toast from sonner (already used in project)
  - [ ] Map error codes to user-friendly messages:
    - NETWORK_ERROR: "Connection lost. Retrying..."
    - SERVER_ERROR: "Server temporarily unavailable. Switching to standard mode."
    - TIMEOUT: "Request timed out. Using standard mode."
    - PARSE_ERROR: "Received invalid data. Using standard mode."
  - [ ] Show toast with error message
  - [ ] Include retry count in message during retries: "Retrying (2/3)..."
  - [ ] Write tests for toast messages

- [ ] Task 8: Handle malformed SSE events gracefully (AC: 8)
  - [ ] Wrap JSON.parse in try/catch in all event listeners
  - [ ] Log parse errors: `console.error('Failed to parse SSE event:', error, rawData)`
  - [ ] Continue streaming (don't close connection on single bad event)
  - [ ] Track parse error count, close if > 5 consecutive errors
  - [ ] Don't call callbacks on malformed events (skip)
  - [ ] Write tests for malformed event handling

- [ ] Task 9: Clean up connection on navigation away (AC: 9)
  - [ ] Use useEffect cleanup in useStreaming hook
  - [ ] In cleanup function: `eventSource?.close()`
  - [ ] Remove event listeners before closing
  - [ ] Ensure no error logs or console warnings on cleanup
  - [ ] Test by navigating away mid-stream (manual test)
  - [ ] Write tests for cleanup

- [ ] Task 10: Add timeout detection (AC: 6, 11)
  - [ ] Add timer in useStreaming: `let timeoutId: NodeJS.Timeout`
  - [ ] Set 30-second timeout: `timeoutId = setTimeout(() => { ... }, 30000)`
  - [ ] Reset timeout on each received event
  - [ ] On timeout, close connection and trigger fallback
  - [ ] Show toast: "Request timed out. Using standard mode."
  - [ ] Clear timeout on cleanup
  - [ ] Write tests for timeout detection

- [ ] Task 11: Persist fallback preference (AC: 12)
  - [ ] On SET_FALLBACK_MODE action, save to sessionStorage:
    - `sessionStorage.setItem('orch_use_fallback', 'true')`
  - [ ] On ChatProvider mount, check sessionStorage:
    - `const useFallback = sessionStorage.getItem('orch_use_fallback') === 'true'`
  - [ ] Initialize state with persisted value
  - [ ] Clear on session end (sessionStorage auto-clears on tab close)
  - [ ] Write tests for persistence

- [ ] Task 12: Add fallback indicator to UI (AC: 3)
  - [ ] Update ChatInterface to show mode indicator
  - [ ] If useFallbackMode is true, show badge: "Standard Mode"
  - [ ] Optional: Show icon next to input (no streaming icon)
  - [ ] Use subtle styling (don't distract user)
  - [ ] Write tests for UI indicator

- [ ] Task 13: Write comprehensive tests (AC: All)
  - [ ] Test SSE connection failure triggers retry
  - [ ] Test exponential backoff delays (1s, 2s, 4s)
  - [ ] Test max 3 retries then fallback
  - [ ] Test fallback mode uses POST endpoint
  - [ ] Test malformed event handling (doesn't crash)
  - [ ] Test timeout detection and fallback
