...
- [ ] Create chat router in `orchestratai_api/src/api/routes/chat.py` (AC: 1, 2, 3)
  - [ ] Create APIRouter with prefix="/api" and tags=["chat"]
  - [ ] Define POST `/chat` endpoint with `ChatRequest` as request body
  - [ ] Set `ChatResponse` as response_model for automatic validation
  - [ ] Import and call `generate_mock_response` from mock_data service
  - [ ] Return ChatResponse object from endpoint handler

- [ ] Update main.py to register chat router (AC: 1, 6)
  - [ ] Import chat router from `api.routes.chat`
  - [ ] Include router in FastAPI app: `app.include_router(chat_router)`
  - [ ] Verify router is registered before any middleware

- [ ] Configure CORS middleware in main.py (AC: 7)
  - [ ] Import `CORSMiddleware` from fastapi.middleware.cors
  - [ ] Add CORS middleware with allow_origins=["http://localhost:3000"]
  - [ ] Set allow_credentials=True, allow_methods=["*"], allow_headers=["*"]
  - [ ] Add middleware after router registration

- [ ] Add request validation error handling (AC: 5)
  - [ ] FastAPI automatically returns 422 for Pydantic validation errors
  - [ ] Verify 400/422 response format includes field-level error details
  - [ ] Test with invalid request (missing fields, wrong types)

- [ ] Write integration tests for chat endpoint (AC: 8)
  - [ ] Test successful chat request with valid message and session_id
  - [ ] Test response matches ChatResponse schema (all fields present)
  - [ ] Test routing to different agents based on message keywords
  - [ ] Test invalid request body returns 422 with error details
  - [ ] Test missing required fields returns proper error
  - [ ] Test message length validation (max 2000 chars)

- [ ] Verify OpenAPI documentation (AC: 6)
  - [ ] Start backend server: `cd orchestratai_api && uv run uvicorn src.main:app --reload`
  - [ ] Access http://localhost:8000/docs
  - [ ] Verify `/api/chat` endpoint appears with correct request/response schemas
  - [ ] Test endpoint directly from Swagger UI

- [ ] Manual testing with curl (AC: 8)
  - [ ] Test billing routing: curl with message "What are your pricing tiers?"
  - [ ] Test technical routing: curl with message "I'm getting an error"
  - [ ] Test policy routing: curl with message "What's your refund policy?"
  - [ ] Test orchestrator fallback: curl with generic message "Hello"
  - [ ] Verify all responses include agent, confidence, logs, metrics
