...
- [ ] Create AgentMetrics component (AC: 1, 2, 3, 4, 5, 7, 8)
  - [ ] Create file `orchestratai_client/src/components/panels/agent-metrics.tsx`
  - [ ] Mark as Client Component with `"use client"` directive
  - [ ] Create AgentMetricsProps interface:
    ```typescript
    ...
    ```
  - [ ] Create formatters for each metric type
  - [ ] Render metrics in grid layout with labels

- [ ] Create number formatting utilities (AC: 1, 2, 3, 8)
  - [ ] Create file `orchestratai_client/src/lib/utils/format-metrics.ts`
  - [ ] Add `formatTokens(tokens: number): string` function
    - Returns: `"${tokens.toLocaleString()} tokens"`
    - Example: 1234 → "1,234 tokens"
  - [ ] Add `formatCost(cost: number): string` function
    - Returns: `"$${cost.toFixed(4)}"`
    - Example: 0.00234 → "$0.0023"
  - [ ] Add `formatLatency(latency: number): string` function
    - Returns: `"${latency.toLocaleString()}ms"`
    - Example: 1234 → "1,234ms"

- [ ] Create cache status indicator (AC: 4)
  - [ ] Add CacheStatusIcon subcomponent in agent-metrics.tsx
  - [ ] Map cache status to icon and color:
    - hit → green checkmark icon
    - miss → yellow dash icon
    - none → gray circle icon
  - [ ] Use Lucide React icons: CheckCircle, MinusCircle, Circle
  - [ ] Add tooltip with status text (use shadcn Tooltip if available)

- [ ] Update AgentCard to include AgentMetrics (AC: 5, 7)
  - [ ] Import AgentMetrics component in agent-card.tsx
  - [ ] Update AgentCardProps to include metrics fields
  - [ ] Add metrics prop to interface:
    ```typescript
    metrics: {
      tokens: number;
      cost: number;
    ...
    cacheStatus: "hit" | "miss" | "none";
    ```
  - [ ] Render AgentMetrics in CardContent below strategy badge
  - [ ] Add separator between strategy and metrics using shadcn Separator

- [ ] Add metrics grid layout (AC: 5)
  - [ ] Use grid layout for metrics display
  - [ ] 2 columns on desktop, 1 column on mobile
  - [ ] Each metric has label + value structure
  - [ ] Labels use text-text-tertiary color
  - [ ] Values use text-text-primary color
  - [ ] Apply spacing tokens: gap-2, py-2

- [ ] Write unit tests for format utilities (AC: 9, 10)
  - [ ] Create test file `orchestratai_client/tests/unit/lib/utils/format-metrics.test.ts`
  - [ ] Test formatTokens with 0, 100, 1000, 10000
  - [ ] Test formatCost with 0, 0.0001, 0.0023, 1.2345
  - [ ] Test formatLatency with 0, 50, 1234, 10000

- [ ] Write unit tests for AgentMetrics (AC: 9, 10)
  - [ ] Create test file `orchestratai_client/tests/unit/components/panels/agent-metrics.test.tsx`
  - [ ] Test renders all metrics with correct formatting
  - [ ] Test cache hit shows green icon
  - [ ] Test cache miss shows yellow icon
  - [ ] Test cache none shows gray icon
  - [ ] Test zero values display correctly

- [ ] Run test coverage validation (AC: 10)
