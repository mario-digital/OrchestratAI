...
- [ ] Task 1: Install EventSource polyfill (AC: 11)
  - [ ] Run `bun add eventsource-polyfill`
  - [ ] Import polyfill in hook file (conditional for browser only)
  - [ ] Verify polyfill works in Safari < 12 (BrowserStack or manual test)

- [ ] Task 2: Create useStreaming hook file structure (AC: 1, 9)
  - [ ] Create `hooks/use-streaming.ts` file
  - [ ] Define hook signature with callback parameters
  - [ ] Define return type: `{ sendStreamingMessage, isStreaming }`
  - [ ] Add useState for isStreaming flag
  - [ ] Add useRef for EventSource instance (persist across renders)
  - [ ] Add useCallback for sendStreamingMessage function

- [ ] Task 3: Implement EventSource connection (AC: 1)
  - [ ] Construct SSE endpoint URL with API base from env
  - [ ] Use POST method via fetch to initiate SSE connection (EventSource only supports GET)
  - [ ] Alternative: Send message via GET query params (less secure, size limited)
  - [ ] Recommendation: Use POST /api/chat/stream with request body, then establish EventSource to dedicated session URL
  - [ ] Create EventSource instance: `new EventSource(url)`
  - [ ] Set isStreaming to true on connection open
  - [ ] Handle connection errors in onerror event

- [ ] Task 4: Implement message_chunk event handler (AC: 3)
  - [ ] Add event listener: `eventSource.addEventListener('message_chunk', handler)`
  - [ ] Parse event data: `JSON.parse(e.data)`
  - [ ] Accumulate chunks in local variable: `accumulatedMessage += data.content`
  - [ ] Call onChunk callback with accumulated message (not individual chunk)
  - [ ] Handle JSON parse errors (log to console, continue stream)
  - [ ] Write tests for chunk accumulation

- [ ] Task 5: Implement agent_status event handler (AC: 4)
  - [ ] Add event listener: `eventSource.addEventListener('agent_status', handler)`
  - [ ] Parse event data: `JSON.parse(e.data)`
  - [ ] Extract agent and status from data
  - [ ] Call onAgentUpdate(agent, status) callback
  - [ ] Handle JSON parse errors gracefully
  - [ ] Write tests for agent status updates

- [ ] Task 6: Implement retrieval_log event handler (AC: 5)
  - [ ] Add event listener: `eventSource.addEventListener('retrieval_log', handler)`
  - [ ] Parse event data: `JSON.parse(e.data)`
  - [ ] Convert to LogEntry object (validate with Zod schema if needed)
  - [ ] Call onLog(logEntry) callback
  - [ ] Handle JSON parse errors gracefully
  - [ ] Write tests for log entry handling

- [ ] Task 7: Implement done event handler (AC: 6, 7)
  - [ ] Add event listener: `eventSource.addEventListener('done', handler)`
  - [ ] Parse event data: `JSON.parse(e.data)`
  - [ ] Call onComplete(metadata) callback with final metrics
  - [ ] Close EventSource connection: `eventSource.close()`
  - [ ] Set isStreaming to false
  - [ ] Write tests for stream completion

- [ ] Task 8: Implement cleanup on unmount (AC: 8)
  - [ ] Use useEffect with cleanup function
  - [ ] In cleanup: Check if EventSource exists
...
- [ ] Task 9: Add error handling (AC: 10)
  - [ ] Add onerror event handler to EventSource
...
- [ ] Task 10: Add TypeScript types (AC: 2, 3, 4, 5, 6)
  ...
  - [ ] Import types from lib/types and lib/enums
  - [ ] Ensure full type safety (no `any` types)

- [ ] Task 11: Write comprehensive tests (AC: All)
  - [ ] Test EventSource connection creation
  - [ ] Test message chunk accumulation
  - [ ] Test agent status event handling
  - [ ] Test log entry event handling
  - [ ] Test done event and connection cleanup
  - [ ] Test error handling
  - [ ] Test unmount cleanup
  - [ ] Mock EventSource API (jest mock or MSW)
  - [ ] Achieve 90%+ coverage
