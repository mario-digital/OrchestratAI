...
- [ ] Task 1: Verify SSE event handlers (AC: 3)
  - [ ] Open `orchestratai_client/src/hooks/use-streaming.ts`
  - [ ] Review existing event handling logic
  - [ ] Verify event types match backend SSE messages:
    - `agent_status`
    - `retrieval_log`
    - `message_chunk`
    - `done`
  - [ ] No changes needed if event types already match
  - [ ] If mismatches found, update event type handling
  - [ ] Ensure error events handled gracefully
